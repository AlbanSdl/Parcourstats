...
import {
  ...
  DatasetGraphEntry,
  ComputedGraphEntry,
  PreviousBasedComputedGraphEntry,
...
export class WishFragment extends Page<Home, Adapter<Formation>> {
  ...
  protected async onCreate(from: Page<Home, Adapter<Formation>>) {
    ...
    const sessionSetter = createElement({
      classes: ["complement", "session-set"],
    });
    title.append(complement, sessionSetter);
  ...
  protected onCreated(): void {
    this.data.then(async (data) => {
      const sessions = [
        ...data.asList.find((formation) => formation.name === this.wishName)
          .sessions,
      ].sort((session) => session.year);
      let wish = sessions[0];
      ...
      const update = (wish?.global?.map((g) => g.time) ?? [])
        ...
        .slice(-1)[0];
      (update === undefined
        ? this.getLocale("wish.rank.unknown")
        : Promise.resolve(
        ...
        (dateString) =>
          (this.root.querySelector(
            ".container .title .complement.update .value"
          ).textContent = dateString)
      );

      const dropdown = new Dropdown<Formation["sessions"][number]>({
        label: this.getLocale("wish.session.set"),
        onSelect: (session) => {
          if (this.currentSession != session.year) this.loadSession(session);
        },
        values: Object.fromEntries(
          sessions.map((session) => [session.year, session])
        ),
        parent: this.root.querySelector(
          ".container .title .complement.session-set"
        ),
        disabled: false,
      });

      await this.loadSession(wish);
      dropdown.select(wish, false);

      this.root.toggleAttribute("loading", false);
    });
  }
  private async loadSession(session: Formation["sessions"][number]) {
    this.currentSession = session.year;

    const userData = session?.user ?? [],
      globalData = session?.global ?? [];
    let overall: Promise<string>;
    let displayedRank: Formation["user"] extends Array<infer R>
      ? R
      : never | undefined;
    let displayGlobalData: Formation["global"] extends Array<infer R>
      ? R
      : never | undefined;
    if (userData[userData.length - 1]?.queued > 0) {
      ...
      overall = this.getLocale(
        userData.length === 1
          ? "wish.rank.unknown.accepted"
    ...
    if (!!displayedRank && !!displayGlobalData) {
      this.root
        .querySelector(".container > .overview > .rank.user")
        ?.setAttribute("position", displayedRank.queued.toString());
      this.root
        .querySelector(".container > .overview > .rank.abs")
        ?.setAttribute("position", session.absolute.toString());
    }
    if (!!displayGlobalData) {
      if (!!displayGlobalData.all)
        this.root
          .querySelector(".container > .overview > .rank.all")
          ?.setAttribute("position", displayGlobalData.all.toString());
      if (!!displayGlobalData.last)
        this.root
          .querySelector(".container > .overview > .rank.last")
    ...
    if ((displayGlobalData?.last ?? 0) < 1)
      ...
      this.root.toggleAttribute("no-data", false);
      const userRank = new DatasetGraphEntry(
        this.getLocale("wish.rank.user"),
        "user-rank"
      );
      userRank.add(
        new Map(
          userData
            ...
            .map((entry) => [Math.trunc(entry.time / 864e5), entry.queued])
        )
      );
      const allApplications = new DatasetGraphEntry(
        this.getLocale("wish.rank.all"),
        "app-all"
      );
      allApplications.add(
        new Map(
          globalData.map((entry) => [Math.trunc(entry.time / 864e5), entry.all])
        )
      );
      const lastAcceptedRank = new DatasetGraphEntry(
        this.getLocale("wish.rank.last"),
        "app-last"
      );
      lastAcceptedRank.add(
        new Map(
      ...
      const renouncingPeopleBehindUser = new ComputedGraphEntry(
        this.getLocale("wish.graph.people.after"),
        "user-after",
        (_, all, user) => all - user,
        allApplications,
        userRank
      ...
      await this.graph.add(
        ...
        allApplications,
        lastAcceptedRank,
        renouncingPeopleBehindUser
      );

      const size = session.size || 1;
      const userRankAdvancementSpeed = new PreviousBasedComputedGraphEntry(
        this.getLocale("wish.graph.speed.user"),
        "speed-user-rank",
        (_, pre, current) => (pre === undefined ? pre : (pre - current) / size),
        userRank
      );
      const formationAdvancementSpeed = new PreviousBasedComputedGraphEntry(
        this.getLocale("wish.graph.speed.last"),
        "speed-app-last",
        (_, pre, current) => (pre === undefined ? 0 : (current - pre) / size),
        lastAcceptedRank
      );
      const queueShrinkSpeed = new PreviousBasedComputedGraphEntry(
        this.getLocale("wish.graph.speed.all"),
        "speed-app-all",
        (_, pre, current) => (pre === undefined ? 0 : (pre - current) / size),
        allApplications
      );
      this.speedGraph.add(
        userRankAdvancementSpeed,
        formationAdvancementSpeed,
        queueShrinkSpeed
      );

      const recordDates = new Set(
        userData
          .map((userRecord) => userRecord.time)
          .concat(globalData.map((globalRecord) => globalRecord.time))
          .map((timestamp) => Math.trunc(timestamp / 86400000))
          .sort((a, b) => b - a)
      );
      const getSignedNumber = (n: number) =>
        n < 0 || !isFinite(n) ? n.toString() : `+${n.toString()}`;
      const fast = (value: number, diff: number) =>
        isFinite(diff) && !!Math.trunc((20 * Math.abs(diff)) / (value || 1));

      const table = this.root.querySelector(".container > .updates");
      while (table.children.length > 1) table.lastElementChild.remove();
      scheduleRepeating<PreviousRecord>(
        ...[...recordDates].map((record) => {
          const glob = globalData.find(
            (gl) =>
              Math.trunc(new Date(gl.time).getTime() / 86400000) === record
          );
          const user = userData.find(
            (us) =>
              Math.trunc(new Date(us.time).getTime() / 86400000) === record
          );
          if ((user?.queued ?? -1) < 0) return null;
          return (previous: PreviousRecord) => {
            const previousRankElement = previous?.row?.children?.item(
                1
              ) as HTMLElement,
              previousLastElement = previous?.row?.children?.item(
                2
              ) as HTMLElement,
              previousAllElement = previous?.row?.children?.item(
                3
              ) as HTMLElement;
            ...
            if (!!previousAllElement) {
              ...
              if (fast(glob?.all, diff)) previousAllElement.setIcon(Icon.TREND);
            }
